...
if(!defined('INITIALIZED'))
	exit;
		...
		$main_content .= '<img src="layouts/guilcera_layout/img/titles/t_characters.png"/><p>';
...
if(!empty($name))
{
	...
	if($player->isLoaded())
	{
		...
		$main_content .= '<tr bgcolor="' . $bgcolor . '"><td width="20%">Name:</td><td>' . htmlspecialchars($player->getName()) . '';
		if($player->isBanned() || $account->isBanned())
			$main_content .= '<span style="color:red"> <b>Banned</b></span>';
		if($player->isNamelocked())
			$main_content .= '<span style="color:red"> <b>Namelocked</b></span>';
			/*
...
*/
		...
		$main_content .= '<tr bgcolor="' . $bgcolor . '"><td>Vocation:</td><td>' . htmlspecialchars(Website::getVocationName($player->getVocation(), $player->getPromotion())) . '</td></tr>';
		$bgcolor = (($number_of_rows++ % 2 == 1) ?  $config['site']['darkborder'] : $config['site']['lightborder']);
...
*/
		...
		foreach($player_deaths as $death)
		{
			...
			foreach($killers as $killer)
			{
				...
				{
					if($count <= 4)
						$dead_add_content .= "Killed at Level ".$death['level']." by ";
					elseif($count > 4 and $count < 10)
						$dead_add_content .= "Slain at Level ".$death['level']." by ";
					elseif($count > 9 and $count < 15)
						$dead_add_content .= "Crushed at Level ".$death['level']." by ";
					elseif($count > 14 and $count < 20)
						$dead_add_content .= "Eliminated at Level ".$death['level']." by ";
					elseif($count > 19)
						$dead_add_content .= "Annihilated at Level ".$death['level']." by ";
				}
				elseif($i == $count)
		...
		if(!$player->getHideChar())
		{
			...
			<TR style="color:#5a2800" BGCOLOR="' . $bgcolor . '"><TD><B>Name</B></TD><TD><B>World</B></TD><TD><B>Level</B></TD><TD><b>Status</b></TD><TD><B>&#160;</B></TD></TR>';
			...
			foreach($account_players as $player_list)
			{
				if(!$player_list->getHideChar())
				{
					...
					if(!$player_list->isOnline())
						{
						if (!$player_list->isDeleted())
							{
							if(!$account->isBanned())
							$player_list_status = '';
							else
							$player_list_status = '<font color="red"><b>Banned</b></font>';
							}
						else
						$player_list_status = '<font color="red"><b>Deleted</b></font>';
						}
					else
					...
					$main_content .= '</NOBR></TD><TD WIDTH=15%>'.$config['site']['worlds'][$player_list->getWorld()].'</TD><TD WIDTH=25%>'.$player_list->getLevel().' '.htmlspecialchars($vocation_name[$player_list->getPromotion()][$player_list->getVocation()]).'</TD><TD WIDTH="8%"><b>'.$player_list_status.'</b></TD><TD><TABLE BORDER=0 CELLSPACING=0 CELLPADDING=0><FORM ACTION="?subtopic=characters" METHOD=post><TR><TD><INPUT TYPE="hidden" NAME="name" VALUE="'.htmlspecialchars($player_list->getName()).'"><INPUT TYPE=image NAME="View '.htmlspecialchars($player_list->getName()).'" ALT="View '.htmlspecialchars($player_list->getName()).'" SRC="'.$layout_name.'/img/buttons/sbutton_view.png" BORDER=0 WIDTH=120 HEIGHT=18></TD></TR></FORM></TABLE></TD></TR>';
				}
			}
