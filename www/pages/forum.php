...
// END
$main_content .= '<img src="layouts/guilcera_layout/img/titles/t_forum.png"/><p>';
...
if($action == '')
{
    ...
    $main_content .= '<table width="100%"><tr bgcolor="'.$config['site']['vdarkborder'].'"><td><font color="white" style="font-size:10px"><b>Board</b></font></td><td align="center"><font color="white" style="font-size:10px"><b>Posts</b></font></td><td align="center"><font color="white" style="font-size:10px"><b>Threads</b></font></td><td align="center"><font color="white" style="font-size:10px"><b>Last Post</b></font></td></tr>';
    ...
    foreach($sections as $id => $section)
    {
        ...
        $main_content .= '<tr bgcolor="'.$bgcolor.'"><td><a href="?subtopic=forum&action=show_board&id='.$id.'">'.$section.'</a><br /><small>'.$sections_desc[$id].'</small></td><td><center>'.(int) $counters[$id]['posts'].'</center></td><td><center>'.(int) $counters[$id]['threads'].'</center></td><td>';
        if(isset($last_post['name']))
            $main_content .= date('d.m.y H:i:s', $last_post['post_date']).'<br />by <a style="font-size:11px" href="?subtopic=characters&name='.urlencode($last_post['name']).'">'.$last_post['name'].'</a>';
        else
...
if($action == 'show_board')
{
    ...
    if(isset($last_threads[0]))
    {
        ...
        foreach($last_threads as $thread)
        {
            ...
            $main_content .= '<a href="?subtopic=forum&action=show_thread&id='.$thread['id'].'">'.htmlspecialchars($thread['post_topic']).'</a><br /><small>'.htmlspecialchars(substr(removeBBCode($thread['post_text']), 0, 50)).'...</small></td><td align="center"><a href="?subtopic=characters&name='.urlencode($thread['name']).'">'.$thread['name'].'</a></td><td align="center">'.(int) $thread['replies'].'</td><td align="center">'.(int) $thread['views'].'</td><td>';
            if($thread['last_post'] > 0)
            {
                ...
                $main_content .= date('d.m.y H:i:s', $thread['post_date']).'<br />by <a style="font-size:11px" href="?subtopic=characters&name='.urlencode($thread['name']).'">'.$thread['name'].'</a>';
            $main_content .= '</td></tr>';
        }
        ...
        $main_content .= '<b>No threads in this board.</b>';
}
