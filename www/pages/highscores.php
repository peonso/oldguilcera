...
$main_content .= '<img src="layouts/guilcera_layout/img/titles/t_highscores.png"/><p>';
...
$main_content .= '<TABLE BORDER=0 CELLPADDING=0 CELLSPACING=0 WIDTH=100%><TR><TD><CENTER><H2>Ranking for '.htmlspecialchars($list_name).' on '.htmlspecialchars($world_name).'</H2></CENTER><BR>';

$main_content .= '<br><TABLE BORDER=0 CELLPADDING=4 CELLSPACING=1 WIDTH=100%></TABLE><TABLE BORDER=0 CELLPADDING=4 CELLSPACING=1 WIDTH=100%><TR BGCOLOR="'.$config['site']['vdarkborder'].'"><TD CLASS=whites WIDTH=10%><B>Rank</B></TD><TD CLASS=whites><B>Name</B></TD><TD style="text-align:left" WIDTH=15% CLASS=whites><b>Level</B></TD>';
if($list == "experience")
	$main_content .= '<TD WIDTH=20% CLASS=whites style="text-align:left"><b>Experience</B></TD>';
//$main_content .= '</TR><TR>';
$main_content .= '</TR>';
$number_of_rows = 0;
foreach($skills as $skill)
{
	...
	$main_content .= '<tr bgcolor="'.$bgcolor.'"><td style="text-align:left">'.($offset + $number_of_rows).'</td><td><b><a href="?subtopic=characters&name='.urlencode($skill->getName()).'">'.($skill->getOnline()>0 ? "<font color=\"green\">".htmlspecialchars($skill->getName())."</font>" : "<font>".htmlspecialchars($skill->getName())."</font>").'</a></b></td><td style="text-align:left">'.$value.'</td>';
	if($list == "experience")
		$main_content .= '<td style="text-align:left">'.$skill->getExperience().'</td>';
	$main_content .= '</tr>';
}
