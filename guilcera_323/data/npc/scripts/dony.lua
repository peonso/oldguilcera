...
if string.find(msg, '(%a*)offer(%a*)') and cid == focus then
	selfSay('I am traveling to Peonsville, Taumel, Dodoma, Cona, Xanadu and to Turim.')
end
...
if string.find(msg, 'peonsville') and cid == focus then
	selfSay('Do you want to go to Peonsville for 150 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 1
	return
...
if boat == 1 then
	npcTeleport(cid,244,249,7,150,0)
	focus = 0
	talk_start = 0
	boat = 0
	selfSay('Good travel.')
