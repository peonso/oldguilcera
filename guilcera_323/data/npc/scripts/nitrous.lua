...
if string.find(msg, '(%a*)offer(%a*)') and cid == focus then
	selfSay('I am traveling to Peonsville, Taumel, Dodoma, Cona, Caralin and Turim.')
end
...
if string.find(msg, 'taumel') and cid == focus then
	selfSay('Do you want to go to Taumel for 150 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 2
	return
...
if boat == 2 then
	npcTeleport(cid,311,38,7,150,0)
	focus = 0
	talk_start = 0
	boat = 0
	selfSay('Good travel.')
