...
if string.find(msg, '(%a*)offer(%a*)') and cid == focus then
	selfSay('I am traveling to Peonsville, Dodoma, Cona, Xanadu, Caralin and Turim.')
end
...
if string.find(msg, 'dodoma') and cid == focus then
	selfSay('Do you want to go to Dodoma for 50 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 3
	return
...
if string.find(msg, 'cona') and cid == focus then
	selfSay('Do you want to go to Cona for 50 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 4
	return
...
if string.find(msg, 'xanadu') and cid == focus then
	selfSay('Do you want to go to Xanadu for 150 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 5
	return
...
if string.find(msg, 'turim') and cid == focus then
	selfSay('Do you want to go to Turim for 100 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 8
	return
...
if boat == 3 then
	npcTeleport(cid,172,106,7,50,0)
	focus = 0
	talk_start = 0
	boat = 0
	selfSay('Good travel.')
...
if boat == 5 then
	npcTeleport(cid,416,459,7,150,0)
	focus = 0
	talk_start = 0
	boat = 0
	selfSay('Good travel.')
...
if boat == 8 then
	npcTeleport(cid,594,152,7,100,0)
	focus = 0
	talk_start = 0
	boat = 0
	selfSay('Good travel.')
