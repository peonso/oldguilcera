...
if string.find(msg, '(%a*)offer(%a*)') and cid == focus then
	selfSay('I am traveling to Peonsville, Taumel, Dodoma, Xanadu, Caralin and Turim.')
end
...
if string.find(msg, 'peonsville') and cid == focus then
	selfSay('Do you want to go to Peonsville for 100 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 1
	return
...
if string.find(msg, 'dodoma') and cid == focus then
	selfSay('Do you want to go to Dodoma for 10 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 3
	return
...
if string.find(msg, 'caralin') and cid == focus then
	selfSay('Do you want to go to Caralin for 50 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 7
	return
...
if string.find(msg, 'turim') and cid == focus then
	selfSay('Do you want to go to Turim for 150 gold pieces?')
	talk_start = os.clock()
	talkcount = 4
	boat = 8
	return
...
if boat == 1 then
	npcTeleport(cid,244,249,7,100,0)
	focus = 0
	talk_start = 0
	boat = 0
	selfSay('Good travel.')
...
if boat == 3 then
	npcTeleport(cid,172,106,7,10,0)
	focus = 0
	talk_start = 0
	boat = 0
	selfSay('Good travel.')
...
if boat == 7 then
	npcTeleport(cid,196,556,7,50,0)
	focus = 0
	talk_start = 0
	boat = 0
	selfSay('Good travel.')
...
if boat == 8 then
	npcTeleport(cid,594,152,7,150,0)
	focus = 0
	talk_start = 0
	boat = 0
	selfSay('Good travel.')
