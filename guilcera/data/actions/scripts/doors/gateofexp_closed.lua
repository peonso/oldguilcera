...
function onUse(cid, item, frompos, item2, topos)
	...
	if(isLevelDoor and getPlayerLevel(cid) < (item.actionid-1000)) then
		...
		doSetItemSpecialDescription(item.uid, "You need be level " .. level .. " or higher to open this door.")
		canEnter = false
	end
	...
	if (not canEnter and getPlayerAccess(cid) == 0 and isVocationDoor) then
		doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Only the worthy may pass.")
		return TRUE
	end


	if (not canEnter and getPlayerAccess(cid) == 0 and isLevelDoor) then
		local level = item.actionid - 1000
		doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "You need be level " .. level .. " or higher to open this door.")
		return TRUE
	end
