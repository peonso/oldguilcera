...
itemstom = {}
itemstom[0] = {name = 'worm', id = 3976, subtype = 1, sell = 1, buy = -1, stackable = 1}
itemstom[1] = {name = 'torch', id = 2050, subtype = -1, sell = 2, buy = -1, stackable = 0}
itemstom[2] = {name = 'shovel', id = 2554, subtype = -1, sell = 50, buy = -1, stackable = 0}
itemstom[3] = {name = 'scythe', id = 2550, subtype = -1, sell = 50, buy = -1, stackable = 0}
itemstom[4] = {name = 'rope', id = 2120, subtype = -1, sell = 50, buy = -1, stackable = 0}
itemstom[5] = {name = 'bag', id = 1995, subtype = -1, sell = 5, buy = -1, stackable = 0}
itemstom[6] = {name = 'backpack', id = 2002, subtype = -1, sell = 20, buy = -1, stackable = 0}
itemstom[7] = {name = 'pick', id = 2553, subtype = -1, sell = 50, buy = -1, stackable = 0}
itemstom[8] = {name = 'machete', id = 2420, subtype = -1, sell = 35, buy = 6, stackable = 0}
itemstom[9] = {name = 'rod', id = 2580, subtype = -1, sell = 150, buy = -1, stackable = 0}
itemstom[10] = {name = 'crowbar', id = 2416, subtype = -1, sell = 260, buy = -1, stackable = 0}

local function onActionItem(action)
	if (action == 'buy' and itemstom[_index].sell == -1) then
		selfSay('I\'m not selling it.', _delay * 2)
		_state = 0
		return
	elseif (action == 'sell' and itemstom[_index].buy == -1) then
		selfSay('I\'m not interested.', _delay * 2)
		_state = 0
		return
	end

	DESCRIPTION = getItemDescriptions(itemstom[_index].id)
	amount = ''
	...
	cost = itemstom[_index].buy
	if (_count > 1) then
		cost = itemstom[_index].buy * amount
	end

	if (action == 'buy') then
		cost = itemstom[_index].sell
		if (_count > 1) then
			cost = itemstom[_index].sell * amount
		end
	end
	...
	elseif (getNpcFocus() ~= cid) then
		...
		elseif (_state == 1) then
			if (msgcontains(msg, 'yes')) then
				if (doPlayerRemoveMoney(cid, itemstom[_index].sell * _count) == 1) then
 
					if itemstom[_index].stackable == TRUE then
						local _stacks = math.floor(_count/100)
						_count = _count - _stacks*100
						if _stacks > 0 then
							for i = 1, _stacks do
								doPlayerAddItem(cid, itemstom[_index].id, 100)
      							end
    						end
    						if _count > 0 then
							doPlayerAddItem(cid, itemstom[_index].id, _count)
    						end

					else
						for i = 1, _count do
							doPlayerAddItem(cid, itemstom[_index].id, itemstom[_index].subtype)
						end
					end
		...
		elseif (_state == 2) then
			if (msgcontains(msg, 'yes')) then
				if (doPlayerRemoveItem(cid, itemstom[_index].id, _count, itemstom[_index].subtype) == 1) then
					doPlayerAddMoney(cid, itemstom[_index].buy * _count)
			...
			for n = 0, table.getn(itemstom) do
				if (msgcontains(msg, itemstom[n].name) or msgcontains(msg, itemstom[n].name .. "s")) then
					_count = getCount(msg)
					_index = n
