...
itemsmishra = {}
itemsmishra[0] = {name = 'bolt', id = 2543, subtype = -1, sell = 3, buy = -1, stackable = 1}
itemsmishra[1] = {name = 'arrow', id = 2544, subtype = -1, sell = 2, buy = -1, stackable = 1}
itemsmishra[2] = {name = 'spear', id = 2389, subtype = -1, sell = 9, buy = -1, stackable = 1}
itemsmishra[3] = {name = 'crossbow', id = 2455, subtype = -1, sell = 500, buy = 120, stackable = 0}
itemsmishra[4] = {name = 'bow', id = 2456, subtype = -1, sell = 350, buy = 100, stackable = 0}
itemsmishra[5] = {name = 'ham', id = 2671, subtype = 1, sell = 6, buy = -1, stackable = 1}
itemsmishra[6] = {name = 'meat', id = 2666, subtype = 1, sell = 3, buy = -1, stackable = 1}

local function onActionItem(action)
	if (action == 'buy' and itemsmishra[_index].sell == -1) then
		selfSay('I\'m not selling it.', _delay * 2)
		_state = 0
		return
	elseif (action == 'sell' and itemsmishra[_index].buy == -1) then
		selfSay('I\'m not interested.', _delay * 2)
		_state = 0
		return
	end

	DESCRIPTION = getItemDescriptions(itemsmishra[_index].id)
	amount = ''
	NAME_TO_SAY = DESCRIPTION.name
	if itemsmishra[_index].subtype == 7 then
	...
	cost = itemsmishra[_index].buy
	if (_count > 1) then
		cost = itemsmishra[_index].buy * amount
	end

	if (action == 'buy') then
		cost = itemsmishra[_index].sell
		if (_count > 1) then
			cost = itemsmishra[_index].sell * amount
		end
	end
...
function onCreatureSay(cid, type, msg)
	...
	elseif (getNpcFocus() ~= cid) then
		...
		elseif (msgcontains(msg, 'name')) then
			_selfSay('I am Mishra. I am selling everything the adventurer needs.')
 			_state = 0
		...
		elseif (_state == 1) then
			if (msgcontains(msg, 'yes')) then
				if (doPlayerRemoveMoney(cid, itemsmishra[_index].sell * _count) == 1) then
 
					if itemsmishra[_index].stackable == TRUE then
						local _stacks = math.floor(_count/100)
						_count = _count - _stacks*100
						if _stacks > 0 then
							for i = 1, _stacks do
								doPlayerAddItem(cid, itemsmishra[_index].id, 100)
      							end
    						end
    						if _count > 0 then
							doPlayerAddItem(cid, itemsmishra[_index].id, _count)
    						end

					else
						for i = 1, _count do
							doPlayerAddItem(cid, itemsmishra[_index].id, itemsmishra[_index].subtype)
						end
					end
		...
		elseif (_state == 2) then
			if (msgcontains(msg, 'yes')) then
				if (doPlayerRemoveItem(cid, itemsmishra[_index].id, _count, itemsmishra[_index].subtype) == 1) then
					doPlayerAddMoney(cid, itemsmishra[_index].buy * _count)
			...
			for n = 0, table.getn(itemsmishra) do
				if (msgcontains(msg, itemsmishra[n].name) or msgcontains(msg, itemsmishra[n].name .. "s")) then
					_count = getCount(msg)
					_index = n
