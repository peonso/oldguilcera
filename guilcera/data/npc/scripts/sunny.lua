...
itemssunny = {}
itemssunny[0] = {name = 'parcel', id = 2595, subtype = -1, sell = 10, buy = -1, stackable = 0}
itemssunny[1] = {name = 'letter', id = 2597, subtype = -1, sell = 5, buy = -1, stackable = 0}
itemssunny[2] = {name = 'label', id = 2599, subtype = -1, sell = 1, buy = -1, stackable = 0}

local function onActionItem(action)
	if (action == 'buy' and itemssunny[_index].sell == -1) then
		selfSay('I\'m not selling it.', _delay * 2)
		_state = 0
		return
	elseif (action == 'sell' and itemssunny[_index].buy == -1) then
		selfSay('I\'m not interested.', _delay * 2)
		_state = 0
		return
	end

	DESCRIPTION = getItemDescriptions(itemssunny[_index].id)
	amount = ''
	NAME_TO_SAY = DESCRIPTION.name
	if itemssunny[_index].subtype == 7 then
	...
	cost = itemssunny[_index].buy
	if (_count > 1) then
		cost = itemssunny[_index].buy * amount
	end

	if (action == 'buy') then
		cost = itemssunny[_index].sell
		if (_count > 1) then
			cost = itemssunny[_index].sell * amount
		end
	end
...
function onCreatureSay(cid, type, msg)
	...
	elseif (getNpcFocus() ~= cid) then
		...
		elseif (msgcontains(msg, 'name')) then
			_selfSay('I am Sunny. I am selling everything the adventurer needs.')
 			_state = 0
		...
		elseif (_state == 1) then
			if (msgcontains(msg, 'yes')) then
				if (doPlayerRemoveMoney(cid, itemssunny[_index].sell * _count) == 1) then
 
					if itemssunny[_index].stackable == TRUE then
						local _stacks = math.floor(_count/100)
						_count = _count - _stacks*100
						if _stacks > 0 then
							for i = 1, _stacks do
								doPlayerAddItem(cid, itemssunny[_index].id, 100)
      							end
    						end
    						if _count > 0 then
							doPlayerAddItem(cid, itemssunny[_index].id, _count)
    						end

					else
						for i = 1, _count do
							doPlayerAddItem(cid, itemssunny[_index].id, itemssunny[_index].subtype)
						end
					end
		...
		elseif (_state == 2) then
			if (msgcontains(msg, 'yes')) then
				if (doPlayerRemoveItem(cid, itemssunny[_index].id, _count, itemssunny[_index].subtype) == 1) then
					doPlayerAddMoney(cid, itemssunny[_index].buy * _count)
			...
			for n = 0, table.getn(itemssunny) do
				if (msgcontains(msg, itemssunny[n].name) or msgcontains(msg, itemssunny[n].name .. "s")) then
					_count = getCount(msg)
					_index = n
