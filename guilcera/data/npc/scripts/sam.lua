...
itemssam = {}
itemssam[0] = {name = 'bolt', id = 2543, subtype = -1, sell = 3, buy = -1, stackable = 1}
itemssam[1] = {name = 'arrow', id = 2544, subtype = -1, sell = 2, buy = -1, stackable = 1}
itemssam[2] = {name = 'spear', id = 2389, subtype = -1, sell = 9, buy = -1, stackable = 1}
itemssam[3] = {name = 'crossbow', id = 2455, subtype = -1, sell = 500, buy = 120, stackable = 0}
itemssam[4] = {name = 'bow', id = 2456, subtype = -1, sell = 350, buy = 100, stackable = 0}

local function onActionItem(action)
	if (action == 'buy' and itemssam[_index].sell == -1) then
		selfSay('I\'m not selling it.', _delay * 2)
		_state = 0
		return
	elseif (action == 'sell' and itemssam[_index].buy == -1) then
		selfSay('I\'m not interested.', _delay * 2)
		_state = 0
		return
	end

	DESCRIPTION = getItemDescriptions(itemssam[_index].id)
	amount = ''
	NAME_TO_SAY = DESCRIPTION.name
	if itemssam[_index].subtype == 7 then
	...
	cost = itemssam[_index].buy
	if (_count > 1) then
		cost = itemssam[_index].buy * amount
	end

	if (action == 'buy') then
		cost = itemssam[_index].sell
		if (_count > 1) then
			cost = itemssam[_index].sell * amount
		end
	end
...
function onCreatureSay(cid, type, msg)
	...
	elseif (getNpcFocus() ~= cid) then
		...
		elseif (msgcontains(msg, 'name')) then
			_selfSay('I am Sam. I am selling everything the adventurer needs.')
 			_state = 0
		...
		elseif (_state == 1) then
			if (msgcontains(msg, 'yes')) then
				if (doPlayerRemoveMoney(cid, itemssam[_index].sell * _count) == 1) then
 
					if itemssam[_index].stackable == TRUE then
						local _stacks = math.floor(_count/100)
						_count = _count - _stacks*100
						if _stacks > 0 then
							for i = 1, _stacks do
								doPlayerAddItem(cid, itemssam[_index].id, 100)
      							end
    						end
    						if _count > 0 then
							doPlayerAddItem(cid, itemssam[_index].id, _count)
    						end

					else
						for i = 1, _count do
							doPlayerAddItem(cid, itemssam[_index].id, itemssam[_index].subtype)
						end
					end
		...
		elseif (_state == 2) then
			if (msgcontains(msg, 'yes')) then
				if (doPlayerRemoveItem(cid, itemssam[_index].id, _count, itemssam[_index].subtype) == 1) then
					doPlayerAddMoney(cid, itemssam[_index].buy * _count)
			...
			for n = 0, table.getn(itemssam) do
				if (msgcontains(msg, itemssam[n].name) or msgcontains(msg, itemssam[n].name .. "s")) then
					_count = getCount(msg)
					_index = n
