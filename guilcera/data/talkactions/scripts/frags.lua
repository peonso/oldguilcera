function onSay(cid, words, param, channel)
	...
	if(getBooleanFromString(getConfigValue('advancedFragList'))) then
		...
		if(skullEnd > 0) then
			...
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_LBLUE, result)
	else
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_LBLUE, "You currently have " .. size.day .. " frags today, " .. (size.day + size.week) .. " this week and " .. (size.day + size.week + size.month) .. " this month.")
		if(size.day > 0) then
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_LBLUE, "Last frag at " .. os.date("%d %B %Y %X", contents.day[1].date) .. " on level " .. contents.day[1].level .. " (" .. contents.day[1].name .. ").")
		end

		local skullEnd = getPlayerSkullEnd(cid)
		if(skullEnd > 0) then
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_LBLUE, "Your " .. (getCreatureSkullType(cid) == SKULL_RED and "red" or "black") .. " skull will expire at " .. os.date("%d %B %Y %X", skullEnd))
		end
	end
