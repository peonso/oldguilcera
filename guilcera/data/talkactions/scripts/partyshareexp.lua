...
function onSay(cid, words, param, channel)
	...
	if(not(members))then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_LBLUE, "You are not in a party.")
		return true
	end

	if(cfg.onlyLeader and not(isPartyLeader(cid)))then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_LBLUE, "Only party leader can enable/disable shared experience.")
		return true
	end

	if(cfg.inFightBlock and hasCreatureCondition(cid, CONDITION_INFIGHT))then
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_LBLUE, "You need to be out of fight.")
		return true
	end
	...
	if(setPartySharedExperience(cid, boolean))then
		...
		doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_LBLUE, "There was an error. Try again in few seconds.")
	end
